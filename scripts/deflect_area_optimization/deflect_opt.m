...
lb = ones(length(C)*2,1)*-5;
ub = ones(length(C)*2,1)*5;
...
try
    ...
    d = -min(u(1:numNodes,1)) + 0.1*area_func(new_V);
catch
...
if c >= sum(doublearea(V, F)) / 2
    c = 0;
else
    c = sum(doublearea(V,F)) / 2 - c;
end
...
[xsol,fval] = fmincon(@f,x0,[],[],[],[],lb,ub,[], options);
